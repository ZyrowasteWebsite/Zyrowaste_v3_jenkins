...
from rag.vectorstore import CHROMA_COLLECTION_NAME  # noqa: E402
...
DEFAULT_RESOURCES_DIR = (
    _DATA_DIR
    if _DATA_DIR.is_dir()
    else PROJECT_ROOT / "_0_Resources"
)
...
_REPORT_NEW = (
    ...
    / "data"
    / "resources"
    / "Project_Report___Biodegradable_Packaging_Material.pdf"
)
...
def ingest(
    ...
    extra_pdf: Path | None = None,
) -> None:
    """Load PDFs, split documents, and store embeddings in ChromaDB."""
    ...
    if not pdf_paths:
        print(
            "No PDFs found under "
            f"{resources_dir} and {extra_pdf}. Nothing to ingest."
        )
        return
    ...
    print(
        f"Ingest complete. Chroma collection "
        f"'{CHROMA_COLLECTION_NAME}' updated at "
        f"{settings.chroma_persist_dir!r}."
    )
